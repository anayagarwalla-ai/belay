...
| Offline Phase 2 matrix | Four local simulation child processes plus controller/watchdogs; frozen 1,000-trajectory schedule, local memory and bounded report storage; no sockets or external service | $0 external | Ctrl+C in the native launcher; wait for owned-worker exits and inspect its result/status teardown receipts. Interrupted evidence remains incomplete. |
...
| Batch consolidation follow-up | Current thread, every 15 minutes within the original 16-check cap; current assignments only, no new work, pauses after the final handoff | Existing Codex account usage; no external service | Pause **Consolidate BELAY current batch** in Codex Automations (`keep-belay-tasks-progressing`) |
| Matrix completion follow-up | Existing fixed 1,000-run job only, every five minutes for at most 288 checks; pauses after complete/incomplete reconciliation and reporting | Existing Codex account usage; token/dollar cost not exposed; no external service | Pause **BELAY full matrix evidence** (`belay-full-matrix-evidence`); this does not stop the native controller, which owns worker cleanup |
