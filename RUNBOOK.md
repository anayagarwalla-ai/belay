...
Current scope is a local/invitation-only Phase 2 short-crossing playtest. **No paid resource, hosting account, card, database, object store, leaderboard, voice service, clip upload endpoint, permanent public URL or production server has been provisioned.** The Sites scaffold is local; `.openai/hosting.json` has no remote project registration.
...
| Browser practice teammates | Operator-started SDK clients in the existing room, at most five empty seats and ten minutes; cooperative/clumsy public-state policy loaded on demand | $0 | **Stop practice bots**, leaving the rope, hiding/closing the tab, owner/bot connection failure, or deadline closes that browser's bot sockets. No process, job, room or remote resource is provisioned. The browser helper requires an operator session; its clients use the existing authenticated seat admission and room cap. |
...
| Current Phase 2 verification | Version 4 candidate `fdc6ae8`: completed 2026-09-08 19:27:56 UTC; 1,000 records, 4,728,413 raw samples and four clean worker exits independently verified. Complete original directory remains local; `reports/phase2-repair/evidence-package/` preserves all 77 files losslessly. | $0 external | All benchmark workers and controller have exited. No automation. Preserve source pins, original artifacts and reconstruction receipts. Local preview is separate; stop it with `npm run dev -- --stop`. |
| Short-crossing verification | Native four-worker 1,000-trajectory campaign launched 2026-09-09 03:58:40 UTC from `bdf967a`; live status in `reports/phase2-parallel-2026-09-09T03-58-40.654Z-93e2e47c-aa9e-46ad-9117-f0d3d397cd8e/status.json`. A first launch was rejected by its memory guard at 0/1,000; preserved separately. | $0 external | Local owned controller and four workers; no automation. Ctrl+C in its launcher requests bounded owned-child shutdown. Do not signal copied PIDs. Preserve interrupted evidence; partial records are not a completed matrix. |
| Rope/catch audio | In-browser synthesized noise/tones, gesture-started, max six one-shot voices, no downloaded assets | $0 | Mute, leave/hide the tab, or close it; audio context closes on component teardown |
...
The Phase 2 commands are `test:phase2-smoke`, `test:phase2-replay`, `test:phase2-stress`, `bench:phase2` and `test:local-load`. The short-crossing matrix uses 180-second crossing and 60-second rescue horizons (the preserved earlier matrix used 600-second crossings) and may take hours; interrupted prefixes are incomplete evidence. Local load defaults to ten rooms and uses process/host memory guards; a guard abort is not a pass. CPU/heap profiles describe allocation churn separately from resident memory.
