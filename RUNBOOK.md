...
| Task coordination follow-up | Current thread, every 15 minutes for at most 16 runs; ends early when the current bounded queue is exhausted or requires a user decision | Existing Codex account usage; no external service | Pause **Keep BELAY tasks progressing** in Codex Automations (`keep-belay-tasks-progressing`) |
