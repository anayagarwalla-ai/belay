...
Existing [BelayRoom](../../server/BelayRoom.ts) supports rooms with two through six physical bodies, immediately deletes the seat mapping in `onLeave`, reports process-wide memory and retains bounded recent timing samples. The physical body remains; same-body seat reservation/reconnection is not implemented. [Samples](../../shared/stats.ts) retains only `TUNING.network.telemetrySamples` observations. Existing [Phase 1 evidence](../../reports/phase1-verification.md) and [Phase 2 socket fixtures](../../reports/local-load-verification.md) are local and explicitly not this production gate. Qualification must exercise six real simulated bodies per room and must not report a recent ring as an hour's percentiles.
