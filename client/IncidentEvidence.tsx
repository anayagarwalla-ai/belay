...
export function IncidentEvidence({ snapshot }: { snapshot: Snapshot }) {
  ...
  return <details className="bench evidence"><summary>Incident and role evidence · {incidents.length} recent {incidents.length === 1 ? 'episode' : 'episodes'}</summary>
    <div className="evidence-content">
      ...
      {incident ? <>
        <p><strong>Incident {incident.id} · {incident.status}</strong> · P{incident.playerIds.map(id => id + 1).join(', P')} · {incident.cascades} cascades<br/>
      ...
      <div className="table-scroll"><table><caption>Adjacent rope spans · tension is a solver estimate</caption>
        <thead><tr><th>Harnesses</th><th>Material length</th><th>Slack</th><th>Tension estimate</th><th>Catch</th></tr></thead>
        <tbody>{spans.map(span => <tr key={span.id}><th scope="row">P{span.a + 1}–P{span.b + 1}</th><td>{span.length.toFixed(2)} m</td><td>{span.slackM.toFixed(2)} m</td><td>{Math.round(span.tensionN)} N</td><td>{span.catchHighlight ? 'Caught span' : '—'}</td></tr>)}</tbody>
      </table></div>
