 ...
 * tapes. Source hashes are retained in the fixture. The original inputs and acceptance ceilings remain unchanged. */
describe.each(fixtures)('known recovery defect ($options.playerCount players)', fixture => {
  ...
  it('replays every recorded frame and retains finite states and energy-correction diagnostics', () => {
    ...
    expect(state.diagnostics.energyProjectionCount).toBeGreaterThanOrEqual(0);
    expect(Number.isFinite(state.diagnostics.maximumEnergyProjectionJ)).toBe(true);
    expect(state.diagnostics.maximumEnergyProjectionJ).toBeGreaterThanOrEqual(0);
  });
  it('stays within the existing 2 cm segment ceiling during focused recovery', () => {
