...
## Current short-crossing check

The user rejected the prior cut as unclear, boring, drab and slow, then approved **“Short, intense crossing first.”** The current pacing target is 60–90 seconds for the experiment. This does not count as one of the six original flat-rope tuning sessions or a passed rescue gate.

1. Open the local preview and click **START CROSSING**. In an empty operator room, three labeled practice bots join before the run starts.
2. Read the goal without opening Test tools. Use **S + A** toward the hut, release Space to walk, and use Space plus movement for a catch/haul. If hanging, move into the wall while the others haul.
3. After one crossing, report: Was the destination obvious before moving? Did movement feel responsive? Was there a moment you wanted to try again or show someone? Describe that moment or say there was none.
4. If it remains boring, or falls feel arbitrary/static roles persist, record a failed cut. Faster bots and a better-looking screen do not pass those questions. Clip capture is still unbuilt.

Bots make immediate solo inspection possible. A four-stranger verdict still requires four recruited strangers under the access-control procedure; bots and friends cannot pass it.
...
5. Both players click START CROSSING. The host stops any automatically added practice bots before the partner joins and before reducing to two climbers. WASD/arrows move relative to the camera; hold Space to brace. Click the canvas after using developer tools. Keyboard blur/tab hiding releases movement.
6. In **Test tools → Operator scene controls**, select **flat**, **2 climbers**, the family, the recorded seed and **30 Hz** authority, then click **Reset and load scene**. Loading resets the world and accepted-input tape while preserving connections and pause state; click **Resume** if paused. Leave devtools closed during free play. Take ten minutes to tug, drag, circle and then try moving together quickly.
...
await BELAY.resume()
await BELAY.startPracticeBots('recovery') // operator only; fill empty seats, same policies as bot:team
BELAY.stopPracticeBots()                // closes only bots owned by this browser
...
1. Start and join the local preview, save any existing evidence, then click **Pause**. In **Test tools → Operator scene controls**, select **rescue**, the team size, seed, family and authority, then **Reset and load scene**. The room stays paused while partners join. This fixture starts a climber at a hole; its immediate fall after resuming is not evidence of natural route pacing.
2. Invite real partners using the protected procedure above. For setup alone, choose **Cooperative** in **Try it with bots** and click **Fill empty seats with bots**. The browser's **Stop practice bots** button releases its bots; they also leave on tab hiding, owner/bot connection failure, or the ten-minute deadline. The terminal alternative is `npm run bot:team -- --mode recovery --seconds 120`; `bad`, `walk`, `static-brace` and `frozen-tail` are available there. Check the connected count and BOT labels, then click **Resume** when ready. Stop bots before reducing the team size; restart the helper after increasing the team if the new seats need bots. Saved measurements remember observed bot seat IDs after they leave, until the measurement window resets. Neither helper counts as human evidence.
