...
4. Use **Save measurements** before resetting. Record whether the warning was visible before collapse, who felt the catch, whether a casualty could act, and whether each helper needed to change action. Receipt/camera-frame timestamps do not prove human attention, and highlighted adjacent spans do not identify a rescuer or culprit. The motion/load counter can credit passive dragging; it cannot establish agency.
5. Use **crossing** for the short bridge fixture, and **flat / 2 climbers** to revisit Gate 1. These are diagnostic scenes, not finished five-to-ten-minute routes: the five recovery-policy crossing smoke runs finished in 18.7–21.93 seconds with zero incidents. Inspect an affected rope segment and the saved state/tape when a catch looks wrong. After **Finish boundary reached** or **Run ended · terminal boundary**, save evidence and use **Reset and load scene** for another run; **Resume** does not restart a completed or failed run.
