...
export class ExpeditionSimulation {
  ...
  private center(p: Vec3) { return { x: p.x, y: p.y + half.y - TUNING.body.harnessHeight, z: p.z }; }
  ...
  private substep(inputs: Move[]) {
    ...
    for (let id = 0; id < this.playerCount; id++) {
      ...
      if (contact.support === 'ground') {
        ...
        v = motorVelocity(v, input, dt * scale, this.family);
        ...
        if (input.brace || toward !== 0 || input.x || input.z) {
          const targetY = input.brace ? 0 : toward * (toward > 0 ? profile.wallClimbSpeed : profile.wallDescendSpeed);
          {
            const effort = clamp(mass * clamp((targetY - v.y) / dt, -profile.wallAcceleration, profile.wallAcceleration)
              + mass * TUNING.gravity, -profile.wallFriction * profile.wallNormalEffortN, profile.wallFriction * profile.wallNormalEffortN);
            v.y += effort / mass * dt;
          }
          const along = { x: input.x + toward * contact.normal.x, z: input.z + toward * contact.normal.z };
          const press = toward > 0 || input.brace ? -profile.wallPressSpeed : Math.max(0, -toward) * profile.wallPressSpeed;
          v.x = (input.brace ? 0 : along.x * profile.wallClimbSpeed) + press * contact.normal.x;
          v.z = (input.brace ? 0 : along.z * profile.wallClimbSpeed) + press * contact.normal.z;
        }
    ...
    for (; iterations < profile.maximumSolverIterations; iterations++) {
      ...
      for (const span of this.spans) {
        ...
        for (let j = 1; j < span.nodes.length - 1; j++) {
          this.constrain(span, span.a, span.nodes[j], j * segment, traction, ropeDelta, dt);
          this.constrain(span, span.nodes[j], span.b, (span.nodes.length - 1 - j) * segment, traction, ropeDelta, dt);
        }
      }
      this.separateBodies();
      for (let i = 0; i < this.nodes.length; i++) {
        ...
        Object.assign(this.nodes[i], body ? { ...projection.position, y: projection.position.y - half.y + TUNING.body.harnessHeight } : projection.position);
        this.diagnostics.contactProjections += projection.projections;
        if (body) contactNormals[i].push(...projection.normals);
      }
      this.routeRope();
      if (iterations + 1 >= profile.solverIterations && this.segmentError() <= profile.solverToleranceM) { iterations++; break; }
    }
    ...
    for (let id = 0; id < this.playerCount; id++) {
      ...
      if (contact.support === 'ground') {
        ...
        if (ropeDelta[id].y <= 0) position.y = contact.solid!.maxY + half.y + TUNING.phase2.collisionSkin;
      }
    ...
    for (let id = this.playerCount; id < this.nodes.length; id++) {
      ...
      for (const normal of contactNormalsAt(this.nodes[id], particleHalf, this.terrain.solids)) {
        const amount = dot(velocity, normal);
  ...
  private constrain(span: Span, a: number, b: number, maximum: number, traction: number[], ropeDelta: Vec3[], dt: number) {
    const p = this.nodes[a], q = this.nodes[b], d = distance(p, q);
    if (d <= maximum) return;
    const direction = { x: (q.x - p.x) / d, y: (q.y - p.y) / d, z: (q.z - p.z) / d };
    const massA = a < this.playerCount ? TUNING.body.mass : TUNING.rope.particleMass;
    ...
    const mobility = (id: number, mass: number, sign: number) => {
      ...
      for (const normal of this.nodeNormals[id]) if (sign * dot(direction, normal) < 0) {
        for (const axis of axes) if (normal[axis]) v[axis] = 0;
      }
      return v;
    };
    const wa = mobility(a, massA, 1), wb = mobility(b, massB, -1);
    const lambdaFor = () => {
      const sum = axes.reduce((value, axis) => value + direction[axis] ** 2 * (wa[axis] + wb[axis]), 0);
      return sum ? (d - maximum) / sum : 0;
    };
    let lambda = lambdaFor();
    const demand = (w: Vec3, mass: number) => lambda * mass / dt * Math.hypot(direction.x * (w.x ? 1 / mass - w.x : 0), direction.z * (w.z ? 1 / mass - w.z : 0));
    ...
    for (const [id, w, sign] of [[a, wa, 1], [b, wb, -1]] as const) for (const axis of axes) {
      const correction = sign * lambda * w[axis] * direction[axis]; this.nodes[id][axis] += correction;
      if (id < this.playerCount) ropeDelta[id][axis] += correction;
      const physicalCorrection = sign * lambda * direction[axis] / (id < this.playerCount ? TUNING.body.mass : TUNING.rope.particleMass);
      if (id < this.playerCount && !w[axis]) this.normalLoadDelta[id][axis] += physicalCorrection;
      if (id === span.a) span.corrections[0][axis] += physicalCorrection;
      if (id === span.b) span.corrections[1][axis] += physicalCorrection;
    }
  }
  ...
  private routeRope() {
    // Non-wrapping bank lips: route an internal particle around the rim, rather than
    // letting a segment join two outside particles through solid ice.
    for (const span of this.spans) for (let j = 1; j < span.nodes.length; j++) {
      const a = this.nodes[span.nodes[j - 1]], b = this.nodes[span.nodes[j]];
      for (const solid of this.terrain.solids) {
        if (!sweepBox(a, b, particleHalf, solid)) continue;
        const index = span.nodes[j] >= this.playerCount ? span.nodes[j] : span.nodes[j - 1];
        if (index < this.playerCount) continue;
        const node = this.nodes[index], outside = b.y < solid.maxY ? b : a;
        node.y = Math.max(node.y, solid.maxY + TUNING.rope.radius + TUNING.phase2.collisionSkin);
        if (outside.z >= solid.maxZ) node.z = solid.maxZ + TUNING.rope.radius + TUNING.phase2.collisionSkin;
        else if (outside.z <= solid.minZ) node.z = solid.minZ - TUNING.rope.radius - TUNING.phase2.collisionSkin;
        if (outside.x >= solid.maxX) node.x = solid.maxX + TUNING.rope.radius + TUNING.phase2.collisionSkin;
        else if (outside.x <= solid.minX) node.x = solid.minX - TUNING.rope.radius - TUNING.phase2.collisionSkin;
      }
    }
  }
  ...
  private updateIncidents(inputs: Move[], before: Vec3[], ropeDelta: Vec3[], dt: number) {
    ...
    if (this.active) {
      for (let id = 0; id < this.playerCount; id++) {
        ...
        const useful = (motion > TUNING.phase2.usefulMotionMps && (loaded || this.states[id] === 'climbing')) || changed && loaded;
        if (useful) this.active.roleActiveSeconds[id] += dt; else this.active.roleIdleSeconds[id] += dt;
        if (input.brace && !changed) this.active.staticHoldSeconds[id] += dt;
      }
