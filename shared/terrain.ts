...
export function physicalTerrain(scene: Scene, seed: number) {
  ...
  if (scene === 'crossing') for (const gap of state.crevasses) {
    const bridge = { id: gap.id, crevasseId: gap.id, minX: -p.bridgeHalfWidth, maxX: p.bridgeHalfWidth,
    ...
    capacities.set(bridge.id, TUNING.body.mass * TUNING.gravity * (p.bridgeCapacityBodyWeights[0]
      + random() * (p.bridgeCapacityBodyWeights[1] - p.bridgeCapacityBodyWeights[0])));
