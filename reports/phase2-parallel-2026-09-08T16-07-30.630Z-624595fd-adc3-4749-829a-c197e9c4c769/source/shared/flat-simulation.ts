...
import { REST, normalizeMove, type Move, type SceneOptions, type SimCounters, type SimulationSnapshot, type Tape, type Vec3 } from './protocol';
...
export class FlatSimulation {
  readonly world: RAPIER.World;
  readonly scene = 'flat' as const;
  readonly playerCount = TUNING.players;
  ...
  constructor(options: SceneOptions = {}) {
    if (options.scene && options.scene !== 'flat' || options.playerCount !== undefined && options.playerCount !== TUNING.players) {
      throw new Error('The Phase 2 engine is not integrated in this schema-only revision.');
    }
    ...
    this.tape = { version: TUNING.version, seed: this.seed, family: this.family, tickHz: this.tickHz,
      scene: this.scene, playerCount: this.playerCount, truncated: false, frames: [] };
  ...
  snapshot(epoch = 0): SimulationSnapshot {
    ...
    return {
      ...
      scene: this.scene, playerCount: this.playerCount,
      terrain: { bounds: { minX: -TUNING.terrain.halfExtent, maxX: TUNING.terrain.halfExtent,
        minZ: -TUNING.terrain.halfExtent, maxZ: TUNING.terrain.halfExtent }, finishZ: null, crevasses: [], bridges: [], ice: [] },
      incidents: [], events: [], diagnostics: emptyPhysicalDiagnostics(),
      run: { status: 'testing', elapsedSeconds: this.tick / this.tickHz, progress: 0 },
      ...
      rope: { points: this.points.map(copy), length: this.length, tension: this.tension, tensionN: this.tensionN,
        spans: [{ id: 0, a: 0, b: 1, startPoint: 0, endPoint: last, length: this.length, tension: this.tension,
          tensionN: this.tensionN, slackM: Math.max(0, this.length - distance(this.points[0], this.points[last])), catchHighlight: false }],
