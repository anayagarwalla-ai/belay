...
import { dot, vectorDistance as distance, projectMotion, supportAt, penetration, blockingNormalsAt, expeditionContactHalf, type Contact } from './contact-geometry';
...
export class ExpeditionSimulation {
  ...
  private constrain(span: Span, a: number, b: number, maximum: number, traction: number[], ropeDelta: Vec3[], dt: number) {
    ...
    const weights = [a, b].map((id, k) => {
      ...
      const normals = blockingNormalsAt(id < this.playerCount ? this.center(this.nodes[id]) : this.nodes[id],
      ...
      for (const normal of normals) for (const axis of axes) if (normal[axis]) w[axis] = 0;
      return w;
    });
