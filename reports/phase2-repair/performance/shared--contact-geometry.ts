...
/** Shared native/PBD envelope; wall cues must query the same support boundary. */
export const expeditionContactHalf: Vec3 = { x: TUNING.body.width / 2 + TUNING.phase2.bodyContactSkinM,
  y: TUNING.body.height / 2 + TUNING.phase2.bodyContactSkinM, z: TUNING.body.depth / 2 + TUNING.phase2.bodyContactSkinM };
...
export function penetration(p: Vec3, half: Vec3, solid: Solid) {
  // Most terrain is far from a node. Reject with scalars before allocating the
  // expanded box used by the exact narrow phase.
  if (p.x <= solid.minX - half.x || p.x >= solid.maxX + half.x
    || p.y <= solid.minY - half.y || p.y >= solid.maxY + half.y
    || p.z <= solid.minZ - half.z || p.z >= solid.maxZ + half.z) return null;
...
export function sweepBox(from: Vec3, to: Vec3, half: Vec3, solid: Solid) {
  if (Math.max(from.x, to.x) <= solid.minX - half.x || Math.min(from.x, to.x) >= solid.maxX + half.x
    || Math.max(from.y, to.y) <= solid.minY - half.y || Math.min(from.y, to.y) >= solid.maxY + half.y
    || Math.max(from.z, to.z) <= solid.minZ - half.z || Math.min(from.z, to.z) >= solid.maxZ + half.z) return null;
  const bounds = limits(solid, half);
...
export function projectMotion(from: Vec3, desired: Vec3, half: Vec3, solids: Solid[]) {
  ...
  for (let pass = 0; pass < TUNING.phase2.collisionSweepPasses; pass++) {
    ...
    for (const solid of solids) {
      ...
      if (!overlap) continue;
      for (const axis of axes) {
        const correction = overlap.normal[axis] * (overlap.depth + TUNING.phase2.contactSeparationM);
    ...
    for (const axis of axes) p[axis] += delta[axis] * first.time + first.normal[axis] * TUNING.phase2.contactSeparationM;
...
export function supportAt(p: Vec3, half: Vec3, solids: Solid[]): Contact {
  for (const solid of solids) {
    ...
    if (foot >= solid.maxY - TUNING.phase2.contactSeparationM && foot - solid.maxY <= TUNING.phase2.groundContactTolerance
...
export function contactNormalsAt(p: Vec3, half: Vec3, solids: Solid[]) {
  const normals: Vec3[] = [];
  const tolerance = TUNING.phase2.collisionSkin * 2;
  for (const solid of solids) {
    if (p.x < solid.minX - half.x - tolerance || p.x > solid.maxX + half.x + tolerance
      || p.y < solid.minY - half.y - tolerance || p.y > solid.maxY + half.y + tolerance
      || p.z < solid.minZ - half.z - tolerance || p.z > solid.maxZ + half.z + tolerance) continue;
    const bounds = limits(solid, half);
    for (const axis of axes) for (const side of [0, 1]) {
...
/** Outside a convex corner is a union of separating half-spaces. Blocking every
 * nearby face instead makes a material point an immovable pin at the lip. Choose
 * the face most compatible with the requested motion separately for each solid. */
export function blockingNormalsAt(p: Vec3, half: Vec3, solids: Solid[], motion: Vec3) {
  const blocked: Vec3[] = [];
  const tolerance = TUNING.phase2.collisionSkin * 2;
  for (const solid of solids) {
    // Match contactNormalsAt's exact broad phase before allocating temporary
    // arrays for this solid. Most banks/bridges are far from the current node.
    if (p.x < solid.minX - half.x - tolerance || p.x > solid.maxX + half.x + tolerance
      || p.y < solid.minY - half.y - tolerance || p.y > solid.maxY + half.y + tolerance
      || p.z < solid.minZ - half.z - tolerance || p.z > solid.maxZ + half.z + tolerance) continue;
    const normals = contactNormalsAt(p, half, [solid]);
    if (!normals.length) continue;
    const best = normals.reduce((a, b) => dot(a, motion) >= dot(b, motion) ? a : b);
    if (dot(best, motion) < 0) blocked.push(best);
  }
  return blocked;
}
